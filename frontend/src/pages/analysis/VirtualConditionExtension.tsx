...
import {
  ApiOutlined, ExperimentOutlined, PlayCircleOutlined, ReloadOutlined,
...
const VirtualConditionExtension: React.FC = () => {
  ...
  const initialConfig = incomingRange ? { ...DEFAULT_CONFIG, speedMin: incomingRange[0], speedMax: incomingRange[1] } : DEFAULT_CONFIG;
  ...
  const generateConditions = () => {
    ...
    const next = Array.from({ length: count }, (_, index) => {
      ...
      return {
        key: String(index + 1), code: `V${String(index + 1).padStart(2, '0')}`,
  ...
  const startPrediction = () => {
    ...
    timer.current = window.setTimeout(() => {
      setConditions((prev) => prev.map((row) => {
        ...
        return { ...row, status: '已完成', thrust: Number((69.5 + row.speed * 0.002 + row.pressure * 1.3 - row.temperature * 0.04).toFixed(1)), temperatureRise: Number((43 + row.speed * 0.0042 + row.temperature * 0.06).toFixed(1)), credibility, risk };
      }));
  ...
  const handleBusinessAction = useCallback((action: BusinessAction) => {
    ...
    const result = { resultType: '虚拟工况预测', resultSummary: `已完成 ${conditions.length} 个虚拟工况预测，高风险工况 ${conditions.filter((row) => row.risk === '高').length} 个`, abnormalRange: '9400～10000 rpm', metrics: ['推力', '温升'] };
    if (action === '生成验证试验') navigate('/experiment/design/intelligent', { state: {
      source: 'virtualCondition', task: incoming?.task, data: incoming?.data, model, result,
      validation: { goal: '确认高转速区域模型预测可靠性', suggestedRange: '9400～9800 rpm', highRiskRange: '9400～10000 rpm', metrics: ['推力', '温升'], recommendedRuns: 5 },
  ...
  }, [conditions, incoming?.data, incoming?.task, model, navigate, predictionStatus]);

  useEffect(() => {
    setContext({
      pageType: 'virtualCondition', pageName: '虚拟工况扩展', projectName: '发动机性能验证', taskName: incoming?.task?.taskName,
      modelName: `${model.modelName} ${model.version}`, resultSummary: predictionStatus === 'completed' ? `已完成 ${conditions.length} 个虚拟工况预测` : generationStatus === 'generated' ? `已生成 ${conditions.length} 个待预测工况` : '等待生成虚拟工况',
      resultReady: predictionStatus === 'completed', onBusinessAction: handleBusinessAction,
    });
    return () => setContext(null);
  }, [conditions.length, generationStatus, handleBusinessAction, incoming?.task?.taskName, model.modelName, model.version, predictionStatus, setContext]);

  const filteredConditions = useMemo(() => conditions.filter((row) => filter === '全部' || (filter === '高风险' ? row.risk === '高' : row.risk === '高' || row.credibility === '低')), [conditions, filter]);
  ...
  const coverageOption = {
    ...
    series: [
      { name: '实测区域', type: 'bar', stack: 'range', data: [6000] }, { name: '高可信虚拟区域', type: 'bar', stack: 'range', data: [800] },
      { name: '谨慎区域', type: 'bar', stack: 'range', data: [600] }, { name: '待验证区域', type: 'bar', stack: 'range', data: [600] },
    ],
  };
  ...
  const sourceName = incoming?.source === 'dataAnalysis' ? `试验数据分析${incoming.task ? ` / ${incoming.task.taskName}` : ''}` : incoming?.source === 'digitalTwin' ? '试验数字孪生' : '';
  ...
  return <div style={{ width: '100%', padding: 16, overflow: 'auto' }}>
    <div style={{ marginBottom: 16 }}><Title level={4} style={{ margin: 0 }}>虚拟工况扩展</Title><Text type="secondary">使用可信数字孪生模型扩展未实测工况，并判断预测风险与可信度</Text></div>
    {sourceName && <Alert type="success" showIcon title={`来源：${sourceName}`} description={`${incoming?.validation ? `待验证区间：${incoming.validation.suggestedRange}；验证目标：${incoming.validation.goal}；` : ''}已携带可信模型：${model.modelName} ${model.version}`} style={{ marginBottom: 16 }} />}
    ...
    <Card title="当前配置" size="small" style={{ marginBottom: 16 }}><Descriptions size="small" column={2} items={[
      { key: 'model', label: '当前模型', children: `${model.modelName} ${model.version}` }, { key: 'status', label: '模型状态', children: <Tag color="green">{model.status}</Tag> },
      { key: 'measured', label: '当前实测范围', children: model.measuredRange ?? '未设置' }, { key: 'target', label: '目标扩展范围', children: `${config.speedMin}～${config.speedMax} rpm` },
      { key: 'variables', label: '扩展变量', children: '转速、温度、压力' }, { key: 'count', label: '虚拟工况', children: `${conditions.length || config.count} 组` },
    ...
    <Card id="business-result" title="虚拟工况与预测结果" size="small">
      {predictionStatus === 'loading' ? <div style={{ padding: '48px 12%' }}><Progress percent={78} status="active" /><Paragraph type="secondary" style={{ textAlign: 'center' }}>正在执行虚拟工况预测与风险标记...</Paragraph></div>
        : generationStatus === 'generated' ? <><Alert type={completed ? 'success' : 'info'} showIcon title={completed ? `已完成 ${conditions.length} 个虚拟工况预测` : `已生成 ${conditions.length} 个虚拟工况，请点击“开始预测”`} style={{ marginBottom: 12 }} /><Tabs activeKey={activeTab} onChange={setActiveTab} items={resultTabs} /></>
          : <Alert type="info" showIcon title="请配置扩展范围并生成虚拟工况" />}
    </Card>

    <Card size="small" style={{ marginTop: 16 }}><div style={{ display: 'flex', justifyContent: 'flex-end' }}><Space>
      <Button disabled={!completed} onClick={() => handleBusinessAction('生成验证试验')}>生成验证试验</Button>
      <Button type="primary" disabled={!completed} onClick={() => handleBusinessAction('加入报告')}>加入报告</Button>
    </Space></div></Card>
